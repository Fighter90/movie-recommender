...
2.  **Primary Function: `loadData()`**
    ...
    -   Inside the `try` block, first `await` the fetch call for `u.item`, read the response with `await response.arrayBuffer()`, decode it as Latin-1 with `new TextDecoder('latin1')`, and pass the decoded text to the `parseItemData` function.
    -   Then, `await` the fetch call for `u.data`, convert the response to text, and pass it to the `parseRatingData` function.
...
3.  **Parsing Function: `parseItemData(text)`**
    ...
    -   It should define an array of 19 genre names, the first being `"unknown"`, matching the 19 flag fields.
...
3.  **Core Logic: `getRecommendations()`**
    -   This is the main function for content-based filtering, triggered by the button click. It must perform the following steps in order:
        ...
        -   **Step 3 (Prepare for Similarity):** Create a JavaScript `Set` from the `genres` array of the `likedMovie`. Create a `candidateMovies` array by filtering the global `movies` array to exclude any movie whose `id` matches the liked movie's ID or whose `title` matches the liked movie's title.
        -   **Step 4 (Calculate Scores):** Create a `scoredMovies` array by mapping over the `candidateMovies`. For each `candidateMovie`, calculate the **Cosine Similarity** between its genre set and the `likedMovie`'s genre set. The formula is `(Size of Intersection) / (sqrt(Size of Liked Genre Set) * sqrt(Size of Candidate Genre Set))`. The resulting objects in the new array should be in the format `{...candidate, score: cosineScore}`.
        -   **Step 5 (Sort by Score):** Sort the `scoredMovies` array in descending order based on the `score`.
        -   **Step 6 (Select Top Recommendations):** Take the first five movies from the sorted array using `.slice(0, 5)`.
...
4.  **Profile-based recommendations:**
    -   Add three `<select>` elements with IDs `watched-1`, `watched-2`, and `watched-3`, populated with the same movie options as `#movie-select`, and add a button labeled "Recommend from profile" that calls `getProfileRecommendations()`.
    -   In `getProfileRecommendations()`, find the three selected movies and build their mean genre vector by averaging the 19-element binary genre vectors position by position.
    -   Score every catalog movie by cosine similarity to the mean profile vector using the same cosine formula as above.
    -   Exclude all three watched movies by both their IDs and their titles.
    -   Sort by descending score, select the Top-5 with `.slice(0, 5)`, and display each recommendation with its score formatted to three decimal places.

#### **6. Verification before output**

Before writing the UI code, print the first parsed movie together with its parsed genres. Also verify that `Star Wars (1977)` has exactly these genres:

```text
Action, Adventure, Romance, Sci-Fi, War
```

The verification should assert that the parsed genre set for `Star Wars (1977)` is exactly `{ "Action", "Adventure", "Romance", "Sci-Fi", "War" }` before producing the UI code.
